...
const repairList = React.createClass({
  ...
  render: function() {
    ...
    const rowsDone = this.state.repairs.filter(repair => this.state.currentCluster == "all" || this.state.currentCluster == repair.cluster_name).filter(repair => repair.state == "DONE").map(repair =>
      <tbody key={repair.id+'-rows'}>
      <TableRow row={repair} key={repair.id+'-head'}
        deleteSubject={this.props.deleteSubject}
        updateStatusSubject={this.props.updateStatusSubject}/>
