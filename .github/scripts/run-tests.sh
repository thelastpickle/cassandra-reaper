...
case "${TEST_TYPE}" in
    ...
    "docker")
        ...
        VERSION=$(printf 'VER\t${project.version}' | mvn help:evaluate | grep '^VER' | cut -f2)
        docker build --build-arg SHADED_JAR=src/server/target/cassandra-reaper-${VERSION}.jar -f src/server/src/main/docker/Dockerfile-slim -t cassandra-reaper:latest .
        docker images
