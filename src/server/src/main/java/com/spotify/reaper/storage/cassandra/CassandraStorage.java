...
import com.spotify.reaper.repair.RepairRun.Builder;
import com.spotify.reaper.repair.RepairRun.RunState;
import com.spotify.reaper.scheduler.RepairSchedule;
import com.spotify.reaper.repair.segment.RepairSegment;
import com.spotify.reaper.repair.segment.RepairSegment.State;
...
public final class CassandraStorage implements IStorage, IDistributedStorage {
  ...
  public RepairRun addRepairRun(Builder repairRun, Collection<RepairSegment.Builder> newSegments) {
    ...
    } catch (InterruptedException | ExecutionExceptionex) {
  ...
  public Collection<RepairRun> getRepairRunsWithState(RunState runState) {
    ...
    return getClusters().stream()
            // Grab all ids for the given cluster name
            .map(Cluster cluster -> getRepairRunIdsForCluster(cluster.getName()))
            // Grab repair runs asynchronously for all the ids returned by the index table
            .flatMap(Collection<UUID> repairRunIds
                    -> repairRunIds.stream()
                            .map(UUID repairRunId -> session.executeAsync(getRepairRunPrepStmt.bind(repairRunId))))
            // wait for results
            ...
            // filter on runState
            .filter(RepairRun repairRun -> repairRun.getRunState() == runState)
  ...
  public Collection<RepairRunStatus> getClusterRunStatuses(String clusterName, int limit) {
    ...
    for (RepairRun repairRun:repairRuns){
      ...
      int segmentsRepaired = (int) segments.stream()
                                      .filter(RepairSegment seg -> seg.getState().equals(RepairSegment.State.DONE))
                                      .count();
  ...
  public Collection<RepairScheduleStatus> getClusterScheduleStatuses(String clusterName) {
    ...
    Collection<RepairScheduleStatus> repairScheduleStatuses = repairSchedules
            .stream()
            .map(RepairSchedule sched -> new RepairScheduleStatus(sched, getRepairUnit(sched.getRepairUnitId()).get()))
  ...
  public boolean renewLead(UUID leaderId) {
    ResultSet lwtResult = session.execute(renewLeadPrepStmt.bind(
