package com.spotify.reaper.scheduler;
...
public class ClusterRepairScheduler {
  ...
  public void scheduleRepairs(Cluster cluster) throws ReaperException {
    ...
    schedulesDiff.keyspacesDeleted().forEach(String keyspace -> deleteRepairSchedule(cluster, keyspace));
    schedulesDiff.keyspacesWithoutSchedules()
        .stream()
        .filter(String keyspace -> keyspaceCandidateForRepair(cluster, keyspace))
        .forEach(String keyspace -> createRepairSchedule(cluster, keyspace, nextActivationStartDate(scheduleIndex.getAndIncrement())));
  }
  ...
  private void deleteRepairSchedule(Cluster cluster, String keyspace) {
    ...
    scheduleCollection.forEach(RepairSchedule repairSchedule -> {
  ...
  private static class ScheduledRepairDiffView {
    ...
    private Set<String> keyspacesThatHaveSchedules(AppContext context, Cluster cluster) {
      ...
      return currentSchedules.stream().map(RepairSchedule repairSchedule -> {
