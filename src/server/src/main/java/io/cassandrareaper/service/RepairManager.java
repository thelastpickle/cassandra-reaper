...
public final class RepairManager implements AutoCloseable {
  ...
  private static boolean takeLead(AppContext context, UUID leaderElectionId) {
    try (Timer.Context cx =
      ...
      if (!result) {
        context
            .metricRegistry
            .counter(MetricRegistry.name(RepairManager.class, "takeLead", "failed"))
            .inc();
      }
      return result;
    }
  ...
  private static boolean renewLead(AppContext context, UUID leaderElectionId) {
    try (Timer.Context cx =
        context
            .metricRegistry
            .timer(MetricRegistry.name(RepairManager.class, "renewLead"))
            .time()) {
      ...
      if (!result) {
        context
            .metricRegistry
            .counter(MetricRegistry.name(RepairManager.class, "renewLead", "failed"))
            .inc();
      }
      return result;
    }
  ...
  private static void releaseLead(AppContext context, UUID leaderElectionId) {
    try (Timer.Context cx =
        context
            .metricRegistry
            .timer(MetricRegistry.name(RepairManager.class, "releaseLead"))
            .time()) {
  ...
  public void resumeRunningRepairRuns() throws ReaperException {
    try {
      Collection<RepairRun> runningRepairRuns =
          repairRunDao.getRepairRunsWithState(RepairRun.RunState.RUNNING);
      Collection<RepairRun> pausedRepairRuns =
          repairRunDao.getRepairRunsWithState(RepairRun.RunState.PAUSED);
  ...
  private void abortAllRunningSegmentsWithNoLeader(Collection<RepairRun> runningRepairRuns) {
    runningRepairRuns.forEach(
        (repairRun) -> {
          Collection<RepairSegment> runningSegments =
              context
                  .storage
                  .getRepairSegmentDao()
                  .getSegmentsWithState(repairRun.getId(), RepairSegment.State.RUNNING);
          Collection<RepairSegment> startedSegments =
              context
                  .storage
                  .getRepairSegmentDao()
                  .getSegmentsWithState(repairRun.getId(), RepairSegment.State.STARTED);
  ...
  private void resumeUnknownRunningRepairRuns(Collection<RepairRun> runningRepairRuns)
      throws ReaperException {
    try {
  ...
  private void abortAllRunningSegmentsInKnownPausedRepairRuns(
      Collection<RepairRun> pausedRepairRuns) {
    try {
      repairRunnersLock.lock();

      pausedRepairRuns.stream()
              ...
              (pausedRepairRun) -> {
                ...
                Collection<RepairSegment> runningSegments =
                    context
                        .storage
                        .getRepairSegmentDao()
                        .getSegmentsWithState(pausedRepairRun.getId(), RepairSegment.State.RUNNING);
                Collection<RepairSegment> startedSegments =
                    context
                        .storage
                        .getRepairSegmentDao()
                        .getSegmentsWithState(pausedRepairRun.getId(), RepairSegment.State.STARTED);
  ...
  private void abortSegmentsWithNoLeader(
    ...
    RepairUnit repairUnit =
        context.storage.getRepairUnitDao().getRepairUnit(repairRun.getRepairUnitId());
  ...
  private void abortSegmentsWithNoLeaderIncremental(
      RepairRun repairRun, Collection<RepairSegment> runningSegments) {
    ...
    try {
      ...
      if (context.storage instanceof IDistributedStorage
        ...
        Collection<RepairSegment> orphanedSegments =
            runningSegments.stream()
                .filter(
                    segment ->
                        !leaders.contains(segment.getId()) && !leaders.contains(segment.getRunId()))
  ...
  private void abortSegmentsWithNoLeaderNonIncremental(
      RepairRun repairRun, Collection<RepairSegment> runningSegments) {
  ...
  public RepairSegment abortSegment(UUID runId, UUID segmentId) throws ReaperException {
    RepairSegment segment =
        context.storage.getRepairSegmentDao().getRepairSegment(runId, segmentId).get();
    try {
      if (null == segment.getCoordinatorHost() || RepairSegment.State.DONE == segment.getState()) {
        RepairUnit repairUnit =
            context.storage.getRepairUnitDao().getRepairUnit(segment.getRepairUnitId());
        // Incremental non subrange repairs will use the run id as leader election id
        // to prevent multiple segments to run at once. Subrange incremental will allow multiple
        // segments.
        UUID leaderElectionId =
            repairUnit.getIncrementalRepair() && !repairUnit.getSubrangeIncrementalRepair()
                ? runId
                : segmentId;
        boolean tookLead;
        if (tookLead =
            takeLead(context, leaderElectionId) || renewLead(context, leaderElectionId)) {
          try {
      ...
      throw new ReaperException(
          "lead is already taken on " + runId + ":" + segmentId, new Exception(error));
    }
  }
  ...
  public RepairRun startRepairRun(RepairRun runToBeStarted) throws ReaperException {
    ...
    LOG.info(
        "Starting a run with id #{} with current state '{}'", runId, runToBeStarted.getRunState());
  ...
  public RepairRun updateRepairRunIntensity(RepairRun repairRun, Double intensity)
      throws ReaperException {
  ...
  private void startRunner(RepairRun run) {
    try {
      ...
      Preconditions.checkState(
          ...
          "there is already a repair runner for run with id "
              + run.getId()
              + ". This should not happen.");

      LOG.debug("scheduling repair for repair run #{}", run.getId());
      try {
        RepairRunner newRunner =
            RepairRunner.create(context, run.getId(), clusterFacade, repairRunDao);
  ...
  public RepairRun abortRepairRun(RepairRun runToBeAborted) throws ReaperException {
    ...
    context
        ...
        .counter(
            MetricRegistry.name(
                RepairManager.class, "repairDone", RepairRun.RunState.ABORTED.toString()))
        .inc();
