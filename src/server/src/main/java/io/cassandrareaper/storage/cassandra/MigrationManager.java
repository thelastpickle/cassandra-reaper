...
final class MigrationManager {
  ...
  static void initializeAndUpgradeSchema(CassandraFactory cassandraFactory, Environment environment,
      ReaperApplicationConfiguration config, Version version,
      CassandraStorageFacade.CassandraMode mode) {

    initializeCassandraSchema(cassandraFactory, environment, config, version);
  }

  static void initializeCassandraSchema(CassandraFactory cassandraFactory, Environment environment,
      ReaperApplicationConfiguration config, Version version) {
    Preconditions.checkState(0 >= Version.parse("2.1").compareTo(version),
    ...
    CqlSession cassandra = cassandraFactory.build(environment.metrics(), environment.lifecycle(),
    ...
    try (Database database =
      ...
      Preconditions.checkState(currentVersion == 0 || currentVersion >= 15,
      ...
      if (currentVersion < migrationRepo.getLatestVersion()) {
        LOG.warn("Starting db migration from {} to {}…", currentVersion,
        ...
        if (15 <= currentVersion) {
          List<String> otherRunningReapers =
              cassandra.execute("SELECT reaper_instance_host FROM running_reapers").all().stream()
                  ...
                  .filter((reaperInstanceHost) -> !AppContext.REAPER_INSTANCE_ADDRESS
                      .equals(reaperInstanceHost))
        ...
        migrate(startVersion, migrationRepo, cassandraFactory, environment,
        ...
        LOG.info(String.format("Keyspace %s already at schema version %d", cassandra.getKeyspace(),
            currentVersion));
      }
    }
  }

  static void migrate(int dbVersion, MigrationRepository repository,
      CassandraFactory cassandraFactory, Environment environment,
      CassandraStorageFacade.CassandraMode mode, String keyspaceName) {
    ...
    for (int i = dbVersion + 1; i <= repository.getLatestVersion(); ++i) {
      ...
      MigrationRepository migrationRepo = new MigrationRepository(migrationRepoPath) {
        ...
        public List getMigrationsSinceVersion(int version) {
          return Collections
              .singletonList((Object) super.getMigrationsSinceVersion(nextVersion - 1).get(0));
        }
      };
      ...
      try (Database database = new Database(cassandra, cassandraFactory.getSessionKeyspaceName())) {
        ...
        Class
            .forName("io.cassandrareaper.storage.cassandra.Migration"
                + String.format("%03d", nextVersion))
            .getDeclaredMethod("migrate", CqlSession.class).invoke(null, cassandra, keyspaceName);
