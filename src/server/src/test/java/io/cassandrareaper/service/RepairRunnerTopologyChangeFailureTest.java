...
public final class RepairRunnerTopologyChangeFailureTest {
  ...
  /**
   * Test: verifyCompleteCoverage (the production method) returns false when a coalesced
   * (multi-range) original segment has its second range entirely uncovered by replacement segments.
   *
   * <p>This exercises the actual production {@code RepairRunner.verifyCompleteCoverage} code path
   * that loops over every original token range independently. The previous version of this test
   * used a broken end-to-end approach where the normal storage flow would correctly create complete
   * replacements, making it impossible for coverage verification to fail. This version calls the
   * production method directly, which is the correct way to test coverage-verification logic.
   */
  @Test
  public void testTopologyChangeDetection_CoverageVerificationFails() throws Exception {
    ...
    // Build a coalesced segment with two ranges: [1, 10) and [20, 30)
    ...
    Segment coalescedSegment =
        Segment.builder().withTokenRanges(Arrays.asList(range1, range2)).build();

    RepairSegment originalSegment =
        RepairSegment.builder(coalescedSegment, repairUnitId)
            .withRunId(runId)
            .withState(RepairSegment.State.NOT_STARTED)
            .withId(UUID.randomUUID())
            .build();

    // Replacements cover range1 fully but provide NO coverage for range2 — this must be detected.
    ...
    // range2 [20, 30) intentionally absent

    // Call the production verifyCompleteCoverage method directly.
    boolean coverageResult = invokeVerifyCompleteCoverage(runner, originalSegment, replacements);

    assertFalse(
        "Production verifyCompleteCoverage must return false when a coalesced range has no coverage",
        coverageResult);
  }
  ...
  private RepairRunner createRepairRunner(UUID runId) throws ReaperException {
    // Use the package-private 5-arg constructor so mockRepairRunService is actually injected.
    AppContext testContext = new AppContext();
    testContext.storage = storage;
    testContext.config = context.config;

    return new RepairRunner(
        testContext, runId, mockClusterFacade, storage.getRepairRunDao(), mockRepairRunService);
  }
