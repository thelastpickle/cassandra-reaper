...
public final class SegmentRunnerTest {
    ...
    public void timeoutTest() throws InterruptedException, ReaperException, ExecutionException,
        ...
        RepairUnit cf = context.storage.getRepairUnitDao()
                .addRepairUnit(RepairUnit.builder().clusterName("reaper").keyspaceName("reaper")
                        .columnFamilies(Sets.newHashSet("reaper")).incrementalRepair(false)
                        .subrangeIncrementalRepair(false).nodes(Sets.newHashSet("127.0.0.1"))
                        .repairThreadCount(1).timeout(segmentTimeout));
        ...
        RepairRun run =
                context.storage.getRepairRunDao()
                        .addRepairRun(
                                RepairRun.builder("reaper", cf.getId()).intensity(0.5)
                                        .segmentCount(1).repairParallelism(PARALLEL).tables(TABLES),
                                Collections
                                        .singleton(RepairSegment.builder(
                                                Segment.builder()
        ...
        context.storage.getClusterDao()
                .addCluster(Cluster.builder().withName(cf.getClusterName())
                        .withPartitioner("Murmur3Partitioner").withSeedHosts(cf.getNodes())
                        .withJmxPort(7199).withState(Cluster.State.ACTIVE).build());

        final UUID runId = run.getId();
        final UUID segmentId = context.storage.getRepairSegmentDao()
                .getNextFreeSegments(run.getId()).get(0).getId();
        ...
        when(jmx.triggerRepair(any(), any(), any(), any(), any(), any(), any(), anyInt()))
                .then((invocation) -> {
                    assertEquals(RepairSegment.State.STARTED, context.storage.getRepairSegmentDao()
                            .getRepairSegment(runId, segmentId).get().getState());

                    future.setValue(executor.submit(new Thread() {
                        @Override
                        public void run() {
                            ((RepairStatusHandler) invocation.getArgument(5)).handle(1,
                                    Optional.of(ProgressEventType.START),
                                    "Repair command 1 has started", jmx);

                            assertEquals(RepairSegment.State.RUNNING,
                                    context.storage.getRepairSegmentDao()
                                            .getRepairSegment(runId, segmentId).get().getState());
                        }
                    }));
                    return 1;
        ...
        context.managementConnectionFactory =
                new JmxManagementConnectionFactory(context, new NoopCrypotograph()) {
                    @Override
                    public JmxCassandraManagementProxy connectImpl(Node host)
        ...
        SegmentRunner sr = SegmentRunner.create(context, clusterFacade, segmentId, COORDS, 100, 0.5,
                PARALLEL, "reaper", ru, TABLES, rr);
        ...
        assertEquals(RepairSegment.State.NOT_STARTED, context.storage.getRepairSegmentDao()
                .getRepairSegment(runId, segmentId).get().getState());
        assertEquals(1, context.storage.getRepairSegmentDao().getRepairSegment(runId, segmentId)
                .get().getFailCount());
    }
    ...
    public void successTest() throws InterruptedException, ReaperException, ExecutionException,
        ...
        final int segmentTimeout = 30;
        RepairUnit cf = storage.getRepairUnitDao()
                .addRepairUnit(RepairUnit.builder().clusterName("reaper").keyspaceName("reaper")
                        .columnFamilies(Sets.newHashSet("reaper")).incrementalRepair(false)
                        .subrangeIncrementalRepair(false).nodes(Sets.newHashSet("127.0.0.1"))
                        .repairThreadCount(1).timeout(segmentTimeout));
        ...
        when(jmx.triggerRepair(any(), any(), any(), any(), any(), any(), any(), anyInt()))
                .then((invocation) -> {
                    assertEquals(RepairSegment.State.STARTED, storage.getRepairSegmentDao()
                            .getRepairSegment(runId, segmentId).get().getState());
                    ...
                    future.setValue(executor.submit(() -> {
                        ...
                        assertEquals(RepairSegment.State.RUNNING, storage.getRepairSegmentDao()
                                .getRepairSegment(runId, segmentId).get().getState());

                        // test an unrelated repair. Should throw exception
                        try {
                            ((RepairStatusHandler) invocation.getArgument(5)).handle(2,
                                    Optional.of(ProgressEventType.ERROR),
                                    "Repair command 2 has failed", jmx);
                        ...
                        ((RepairStatusHandler) invocation.getArgument(5)).handle(1,
                                ...
                                "Repair session succeeded in command 1", jmx);

                        assertEquals(RepairSegment.State.DONE, storage.getRepairSegmentDao()
                                .getRepairSegment(runId, segmentId).get().getState());

                        ((RepairStatusHandler) invocation.getArgument(5)).handle(1,
                                Optional.of(ProgressEventType.COMPLETE),
                                "Repair command 1 has finished", jmx);
        ...
        SegmentRunner sr = SegmentRunner.create(context, clusterFacade, segmentId, COORDS, 5000,
                0.5, PARALLEL, "reaper", ru, TABLES, rr);
        ...
        assertEquals(0, storage.getRepairSegmentDao().getRepairSegment(runId, segmentId).get()
                .getFailCount());
    }
    ...
    public void failureTest() throws InterruptedException, ReaperException, ExecutionException,
        ...
        when(jmx.triggerRepair(any(), any(), any(), any(), any(), any(), any(), anyInt()))
                .then((invocation) -> {
                    ...
                    future.setValue(executor.submit(() -> {
                        ...
                        ((RepairStatusHandler) invocation.getArgument(5)).handle(1,
                                Optional.of(ProgressEventType.ERROR), "Repair command 1 has failed",
                                jmx);

                        assertEquals(RepairSegment.State.NOT_STARTED, storage.getRepairSegmentDao()
                                .getRepairSegment(runId, segmentId).get().getState());
        ...
        assertEquals(RepairSegment.State.NOT_STARTED,
                storage.getRepairSegmentDao().getRepairSegment(runId, segmentId).get().getState());
        assertEquals(2, storage.getRepairSegmentDao().getRepairSegment(runId, segmentId).get()
                .getFailCount());
    }

    @Test
    public void outOfOrderSuccessCass22Test() throws InterruptedException, ReaperException,
            ExecutionException, MalformedObjectNameException, ReflectionException, IOException {
        final IStorageDao storage = new MemoryStorageFacade();
        ...
        RepairUnit cf = storage.getRepairUnitDao()
                .addRepairUnit(RepairUnit.builder().clusterName("reaper").keyspaceName("reaper")
                        .columnFamilies(Sets.newHashSet("reaper")).incrementalRepair(false)
                        .subrangeIncrementalRepair(false).nodes(Sets.newHashSet("127.0.0.1"))
                        .repairThreadCount(1).timeout(30));
    ...
    @Test
    public void outOfOrderFailureTestCass22() throws InterruptedException, ReaperException,
            ExecutionException, MalformedObjectNameException, ReflectionException, IOException {
        final IStorageDao storage = new MemoryStorageFacade();
        final int segmentTimeout = 30;
    ...
    public void parseRepairIdTest() {
        String msg =
                "Repair session 883fd090-12f1-11e5-94c5-03d4762e50b7 for range (1,2] failed with";
    ...
    public void alreadyRunningSegmentRunnerCreationFailure() throws ReaperException {
        ...
        SegmentRunner.create(mock(AppContext.class), mock(ClusterFacade.class), segmentId,
                Collections.emptyList(), 1, 1, RepairParallelism.DATACENTER_AWARE, "clusterName",
                mock(RepairUnit.class), Collections.emptySet(), mock(RepairRunner.class));
    }

    @Test
    public void triggerFailureTest() throws InterruptedException, ReaperException,
            ExecutionException, MalformedObjectNameException, ReflectionException, IOException {
        final IStorageDao storage = new MemoryStorageFacade();
        final int segmentTimeout = 30;
        ...
        assertEquals(RepairSegment.State.NOT_STARTED,
                storage.getRepairSegmentDao().getRepairSegment(runId, segmentId).get().getState());
        assertEquals(1, storage.getRepairSegmentDao().getRepairSegment(runId, segmentId).get()
                .getFailCount());
    }

    @Test
    public void nothingToRepairTest() throws InterruptedException, ReaperException,
            ExecutionException, MalformedObjectNameException, ReflectionException, IOException {
        final IStorageDao storage = new MemoryStorageFacade();
        final int segmentTimeout = 30;
    ...
    @Test
    public void failComputingIntensityDelayTest() throws InterruptedException, ReaperException,
            ExecutionException, MalformedObjectNameException, ReflectionException, IOException {
        ...
        when(mockedRepairSegmentDao.getRepairSegment(any(), any()))
                .thenReturn(Optional.of(segment));

        ClusterFacade clusterFacade = mock(ClusterFacade.class);

        SegmentRunner sr = SegmentRunner.create(context, clusterFacade, Uuids.timeBased(), COORDS,
                5000, 0.5, PARALLEL, "reaper", ru, TABLES, rr);

        assertEquals("Intensity could apply fine although it shouldn't", 0,
                sr.intensityBasedDelayMillis(new Double(1)));
    }

    @Test
    public void clearSnapshotTest() throws InterruptedException, ReaperException,
            ExecutionException, MalformedObjectNameException, ReflectionException, IOException {
        final IStorageDao storage = new MemoryStorageFacade();
        final int segmentTimeout = 30;
    ...
    @Test
    public void clearSnapshotFailTest() throws InterruptedException, ReaperException,
            ExecutionException, MalformedObjectNameException, ReflectionException, IOException {
        final IStorageDao storage = new MemoryStorageFacade();
        final int segmentTimeout = 30;
